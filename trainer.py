...
class Trainer:
    ...
    def fit_nrosd_elr(self, train_dataset: Dataset, val_dataset: Dataset):

        from models import elr_loss
        elr = elr_loss(
            len(train_dataset),
            len(train_dataset.classes),
            lam=self.config['loss_param']['lam'],
        ...
        for epoch in tqdm.trange(self.config["max_epoch"], dynamic_ncols=True):
            ...
            for batch in train_dataloader:
                ...
                # target_loss = self.criterion(output, target).mean()
                distill_loss = distill_criterion(output, output_teacher).mean()
                target_loss = elr(batch['index'].cuda(), output, target).mean()
                loss = target_loss * alpha + distill_loss * (1.0-alpha)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
